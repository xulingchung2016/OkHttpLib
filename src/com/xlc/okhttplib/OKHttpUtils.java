    ...
    public  void request(final Request request, @CacheType final int cacheType, final JsonCallback callback){
        request(request, cacheType, new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
				boolean isture = response.isSuccessful();
...
//            	str = JSON.toJSONString(str);
    ...
    public <T extends Class> String requestAsync(Request request, @CacheType final int cacheType,Class<T> classes) {
            switch (cacheType){
                case CacheType.ONLY_NETWORK:
                    try {
                ...
                case CacheType.NETWORK_ELSE_CACHED:
                    try {
                        String result = requestFromNetworkAsync(request,classes);
                        if(request==null){
                            return requestFromCacheAsync(request,classes);
                        }else{
                            return result;
                ...
                case CacheType.CACHED_ELSE_NETWORK:
                    try {
                        String result = requestFromCacheAsync(request,classes);
                        if(request==null){
                            return requestFromNetworkAsync(request,classes);
                        }else{
                            return result;
    ...
    private  <T extends Class> String requestFromCacheAsync(Request request,Class<T> classes) throws IOException {
        Response response = client.cache().get(request);
        if(response!=null&&response.code()==200){
            return JSON.toJSONString(response.body().toString());
        }
        return null;
    }
    private   <T extends Class> String requestFromNetworkAsync(Request request, Class<T> classes) throws IOException {
